...
Ardublockly.init = function() {
  ...
  Ardublockly.injectBlockly(document.getElementById('content_blocks'),
                            Ardublockly.TOOLBOX_XML, '/ardublockly/blockly/');
...
Ardublockly.importExtraBlocks = function() {
      ...
      for (var catDir in jsonDataObj.categories) {
        ...
        if (extensions) {
          for (var i = 0; i < extensions.length; i++) {
            ...
            blockExtensionJsLoad.src = '/ardublockly/blocks/' + catDir + '/extensions.js';
  ...
  ArdublocklyServer.getJson('/ardublockly/blocks/blocks_data.json', jsonDataCb);
};
...
ArdublocklyServer.sendSketchToServer = function(code, callback) {
    ArdublocklyServer.sendRequest(
        '/arduino/ardublocklycompile', 'POST', 'application/json', {"sketch_code": code}, callback);
};
